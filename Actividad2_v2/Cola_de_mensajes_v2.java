...
public class Cola_de_mensajes_v2 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Cola_v2 colaMensajes=null;
		Escritor_v2[] escritores=null;
		Lector_v2[] lectores=null;
	
		
		/* Creamos la cloa de mensajes, memoria compartida */
		colaMensajes=new Cola_v2();
		
		/* REservamos memoria para lectores y escritores */
		escritores=new Escritor_v2[kNUM_ESCRITORES];
		lectores=new Lector_v2[kNUM_LECTORES];
		...
		for(int i=0; i<kNUM_ESCRITORES; i++) {
			...
			escritores[i]=new Escritor_v2("Escritor"+(i+1), colaMensajes);
			escritores[i].start();
		}
		...
		for(int i=0; i<kNUM_LECTORES; i++) { 
			...
			lectores[i]=new Lector_v2("Lector"+(i+1), colaMensajes);
			lectores[i].start();
		}
